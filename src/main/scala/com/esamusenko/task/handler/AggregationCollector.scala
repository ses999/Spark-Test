...
object AggregationCollector {
  ...
  def collect(df: DataFrame, param: TransformationParams): ResultElem = {
    ...
    val map = df.select(col(param.newColName))
      .filter(col(param.newColName).isNotNull)
      .groupBy(col(param.newColName).cast(StringType))
      .agg(count(param.newColName)).map(r => r.getString(0) -> r.getLong(1))
      .collect()
      .toMap
